...
Three harness-agnostic skills that turn the `ws` CLI into an end-to-end AI workspace
quality system. `ws` is the deterministic oracle (measures, executes primitives, validates
...
| **ws-repo-init** | [`ws-repo-init/SKILL.md`](ws-repo-init/SKILL.md) | Bootstrap a cataloged-but-uninitialized repo from zero to "cataloged + Understand-Anything artifact + commands smoke-validated" in one fast pass | single (smoke) · `--full` delegates to `ws-self-heal` |
...
through non-interactive `ws` commands while the human customizes in chat; the repo-init skill
bootstraps a single cataloged repo (clone + onboarding worktree + `/understand` artifact +
catalog-field derivation + 30s command smoke) so it is ready for the deep pass; the
self-heal skill runs the healthcheck → fix → record → re-check loop the `ws` primitives
define. The three form a pipeline: **init** the workspace → **repo-init** each repo →
**self-heal** for deep convergence.
...
> [!NOTE]
> The install command is **`add`**, not `install`. All three skills carry
...
> `/skill:ws-repo-init`, `/skill:ws-self-heal`).

### Manual / no-CLI fallback
Each `SKILL.md` is plain markdown with no harness-specific runtime. If you can't use the
`skills` CLI, copy or symlink the directories into your agent's skills dir yourself,
e.g. for pi:
```bash
mkdir -p ~/.pi/agent/skills
ln -s "$(pwd)/skills/ws-init"      ~/.pi/agent/skills/ws-init
ln -s "$(pwd)/skills/ws-repo-init" ~/.pi/agent/skills/ws-repo-init
...
- For repo-init: the `/understand` skill installed in the running harness (Point #1 artifact).
