...
1. **Verify Epic Info:**
   ...
   ```bash
   ws ai run provider.issue.get_issue --input '{"key": "ACME-123"}'
   ```
2. **Create Workspace:**
   ...
   - Creates separate Git worktrees in `workspaces/ACME-123/repos/intelligence` and `workspaces/ACME-123/repos/notification`.
   - Checks out a fresh feature branch named `ACME-123`.
