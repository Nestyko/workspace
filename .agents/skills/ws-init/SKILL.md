...
description: >-
  ...
  ws commands. Replaces the TTY-only interactive `ws init` flow for headless agents: the
  human answers setup questions in chat while the agent applies config, seeds the catalog,
  validates, and regenerates AGENTS.md. Scaffolds config + catalog only; does not run
  healthchecks or fix loops (use ws-self-heal for that). Manually invoked only - run it via
  the harness skill command (in pi: /skill:ws-init). On invocation it executes the full
  procedure immediately; no natural-language trigger phrase is required.
...
- in pi: `/skill:ws-init`
- generic: `skill:ws-init`

On invocation, **execute the procedure below immediately**, starting at Step 0. Do not wait
for the user to say a trigger phrase like "initialize ws", and do not ask whether you
should activate - the invocation *is* the activation signal. Ask the human only the
setup questions called out in Step 1.
